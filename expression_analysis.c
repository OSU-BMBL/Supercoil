...
void expression_parse (const char* fn, const char* fn1, const char* fn2, const char* fn3, const char* fn4)
{
	...
	for (i = 0; i < rows; i++)
	{
		...
		for (j = i+1; j < rows; j++)
		{
			...
			if (cnt > po->COL_WIDTH && (abs(which_gene_in_ptt(i)-which_gene_in_ptt(j))<20 || abs(which_gene_in_ptt(i)-which_gene_in_ptt(j))>(geneNum-20)))
			{
				...
				if (cnt > (cnt1*cnt2)/cols)
					...
					g2 = MIN(which_gene_in_ptt(i),which_gene_in_ptt(j));
					if (g2>0 && ((cnt >po->CNT && spearman >po->SPEARMAN) || spearman >po->SPEARMAN+.2))
					{
						g1 = MAX(which_gene_in_ptt(i),which_gene_in_ptt(j));
						if (g1-g2 <=20)
						{
							for (k=g2-1; k<g1-1; k++)
							{
	...
	double	expression_score_smooth[operonNum];
	double	expression_score_smooth_1[operonNum];
	double	expression_score_operon[operonNum];
	double	expression_score_operon_1[operonNum];
	j=0;
	for (i=0; i<geneNum; i++)
	{
		if(gene[i]->IS_operonEnd)
	...
	for (delta = getDelta(expression_score_input)/10; delta < getDelta(expression_score_input)/2; delta += 3*getDelta(expression_score_input)/100)
	{
    		...
    		for(i = 0; i < absorp_count; ++i)
		{
			...
			if (kk==0 || !po->IS_add_nap)
	        	...
	        	/*fprintf(fw2, "%d\t%d\t%ld\t%ld\t%.2f\t%d\t%d\t%ld\t%ld\t%.2f\n", 
					i+1,
					absorp_peaks[i]+1,
					operon[absorp_peaks[i]]->end,       		
					operon[absorp_peaks[i]+1]->start,       		
					expression_score_operon[absorp_peaks[i]],
					kk,
					kk+1,
					gene[kk-1]->end,
					gene[kk]->start,
