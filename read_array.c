...
void read_palindromic (FILE* fp)
{
	size_t 	n = 0;
	char 	*line;
	AllocArray(palin, palindromicNum);
	Palindromic 	*nn;
	int	num=0;
	int	id=0;
	while (getline(&line, &n, fp)>=0)
...
void get_pathway_freq()
{
	...
	{
		AllocArray(id, pathway[i]->capacity);
		AllocArray(id_e, pathway[i]->capacity);
		...
		/*printf ("%d\t%s\t%.2f\t%d\n",i, pathway[i]->component[0], num_col, cols);*/
		free(id);
	}	
}
...
void get_regulon_freq()
{
	...
	{
		AllocArray(id, regulon[i]->capacity);
		AllocArray(id_e, regulon[i]->capacity);
