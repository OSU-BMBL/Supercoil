...
void DyProgramming (FILE *fw)
{
	...
	AllocArray(DpNode, operonInPathwayNum);
	AllocArray(DpNode_cp, operonInPathwayNum);
	AllocArray (DPsolve, 100);
	AllocVar(temp);	
	...
	for(circle=0; circle<1; circle++)
	{
		...
		if(circle>0)
		{
			ReAllocArray(DpNode, operonInPathwayNum);
		...
		dpsolve->score = floor(1000*DpNode[operonInPathwayNum-1]->edgevalue[0]);
...
{
        qsort(el, n, sizeof *el, DP_cmpr);
}


/*search the oepron base on location id*/
int searchwhichoperon(int gauge, Operon **node)
{
        int 	h=0;
...
int searchwhichnewoperon(int gauge, Operon **node)
{
        int 	h=0;
	for(h=0;h<operonInPathwayNum; h++)
        {
...
{
        int 	allregulon[regulonNum];
        int 	allregulonfreq[regulonNum];
        double 	allregulondensity[regulonNum];
        int 	allregulonsum=0;
	int	g=0;
	int	h=0;
	int	u=0;
...
{
        int 	allpathway[pathwayNum];
        int 	allpathwayfreq[pathwayNum];
        double 	allpathwaydensity[pathwayNum];
        int 	allpathwaysum=0;
	int	g=0;
	int	h=0;
	int	u=0;
...
int	which_smallest (int start, int end, double *expression)
{
        int 	j=0;
	int 	jj=10000;
	int	atRich = 1;
	...
	for (j=start; j<end; j++)
        {
			...
			else if (po->IS_AT_rich && (gene[(j==geneNum?:1,j)]->AT+gene[(j==geneNum?:1,j)]->ATlocal)/2<atRich)
			{
                                jj = expression[j-1];
                                kk =j;
			}
...
int 	*uniq_integer(int num, int NUM)
{
        ...
        int *vektor;
        int i=0, rnd;
	AllocArray(vektor, num);
	AllocArray(rand_array, NUM);
	for (i=0;i<NUM; i++)
		rand_array[i]=0;
	srand ( time(NULL) );
	i=0;
